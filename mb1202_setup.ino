...
*/
#define SCL_PIN 0              //Default SDA is Pin5 PORTC for the UNO -- you can set this to any tristate pin  // SCL == PD0 == D3
#define SCL_PORT PORTD 
#define SDA_PIN 1              //Default SCL is Pin4 PORTC for the UNO -- you can set this to any tristate pin  // SDA == PD1 == D2
#define SDA_PORT PORTD
#define I2C_TIMEOUT 200        //Define a timeout of 100 ms -- do not wait for clock stretching longer than this time
...
uint8_t old_addr, new_addr, error;
uint16_t data;
byte input;
...
void loop()
{
  while(1) {  // loop-in-a-loop, so we can use keyword, "continue"
    old_addr = 0;

    Serial.println("searching...");
    for (int i = 0; i < 128; i++) {
      error = start_sensor((byte)(i << 1));
      if (!error) {
        serial_printf("found at address, %i\r\n", i);
        old_addr = (uint8_t)i;
        break;
      }
    }
    ...
    if (0 != old_addr) {
      ...
      // change address?
      input = 'n';
      serial_printf("change address? (y/n): ");
      while (1) {
      ...
      // read new address from user
      String string_addr = "";
      serial_printf("new address: ");
      while(1) {
        input = Serial.read();
        if (isDigit(input)) {
          string_addr += (char)input;
        }
        else if ('\r' == input || '\n' == input) {
          break;
        }
      ...
      // set new address
      new_addr = (uint8_t)string_addr.toInt();
      if (new_addr > 127) {
        serial_printf("ERROR: new address too large, %u\r\n", new_addr);
        continue;
      }
      if (change_address(old_addr << 1, new_addr << 1)) {
        serial_printf("ERROR: failed to set address, %u\r\n", new_addr);
        continue;
      }
      ...
      data = read_sensor(new_addr << 1);
      serial_printf("range, %ucm\r\n", data);
